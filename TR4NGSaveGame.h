...
#define TR4NG_REALHEALTH_OFFSET (offsetof(TR4NG_POSITION,health)-offsetof(TR4NG_POSITION,indicator1))
...
typedef struct TR4NGAmmo    TR4NGAMMO;
...
struct          TR4NGSave
{
    union
    {
        struct
        {
            char                    szSavename [ 24 ];  //  0x18
            char                    cFiller1 [ 51 ];    //  0x33
        ...
        struct
        {
            char                    cBufferMax [ MAX_SAVELENGTH ];
        };
    };
};
...
class CTR4NGSaveGame : public CTR45SaveGame
{
    ...
    public:
        virtual ~CTR4NGSaveGame();

    // Attributes
    protected :
        TR4NGSAVE               *m_pBuffer;
        TR4NGSAVE               *m_pBufferBackup;
    ...
    public:
        ...
        void SetBufferLength(size_t len);
        ...
        WORD *GetTR4NGRealHealthAddress();
        int GetRealHealth ();
...
//
extern TR45_INDICATORS IndicatorsTR4NGTable [ MAX_INDICATORS ];
extern int IndicatorsTR4NGTableCount;
